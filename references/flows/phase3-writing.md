...
2.6. **骚话插入点分析**（参考 [saohua.md](../guides/saohua.md) — 单文件覆盖所有层级）— 根据本章情节要素执行：
...
6. 撰写正文 — **每章必须达到 3000-5000 字**
   ...
   - **骚话插入**：根据步骤 2.6 的分析结果，在对应场景插入骚话——每句骚话前必须有铺垫（≥ 1 段），连续骚话间必须有叙事过渡，风格必须匹配角色人设和题材基线（参考 [saohua.md](../guides/saohua.md) §四质量红线 + §七题材矩阵）
...
10. **深度润色（去除AI味）** — 重点检查并修改：
    ...
    - **优化节奏感**：长句短句交替
    - **细节具象化**：用具体细节替代笼统描述
    - **骚话质量自查**：每句骚话是否匹配角色人设？前后是否有铺垫和过渡？密度是否匹配章节类型？是否有至少一句“截图级”金句？（参考 [saohua.md](../guides/saohua.md) §四质量红线）
...
13.5. **质量趋势刷新**（批次/节点级）
    - 触发时机：每完成 5 章、或一个子 Agent 批次结束、或一个卷（arc）完结时
...
11. 更新 status → "completed"，填入 wordCount + gateScores.reader
12. 立即继续下一章
